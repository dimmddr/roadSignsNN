...
def test_neural_net(min_size=5, max_size=200, step_size=5, init=False, debug=False):
    ...
    if not init:
        ...
        first_ind = 20
        second_ind = first_ind * 10

        first_batch_size = 50
        second_batch_size = 15
        alfa = 0.01
        ...
        second_net = nn.Network(learning_rate=alfa,
                                input_shape=(second_batch_size, SUB_IMG_LAYERS, SUB_IMG_HEIGHT, SUB_IMG_WIDTH),
                                random_state=123
                                )
        ...
        #                       .format(first_batch_size, first_filter_numbers, first_ind))
        first_net.load_params("first_lvl_test_batch_size_50_filter_numbers_100_on_20_image_learn")
        ...
        second_net.save_params("second_lvl_test_batch_size_{}_filter_numbers_{}_on_{}_image_learn"
                               .format(second_batch_size, second_filter_numbers, second_ind))
        # second_net.load_params("second_lvl_test_batch_size_15_filter_numbers_100_on_200_image_learn")

        print("Testing...")
        test_img = np.array(
            list(train_set_without_negatives.keys())[(first_ind + second_ind):(first_ind + second_ind) + 10])
...
def test_all():
    ...
    res = test_neural_net(40, 50, 10, debug=True)
    print(res)
